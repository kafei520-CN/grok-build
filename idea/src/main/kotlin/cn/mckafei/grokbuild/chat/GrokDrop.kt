package cn.mckafei.grokbuild.chat
...
object GrokDrop {
            ...
            object : DropTargetAdapter() {
                override fun dragEnter(event: DropTargetDragEvent) {
                    if (hasFiles(event.transferable)) {
                        event.acceptDrag(dropAction(event.dropAction, event.sourceActions))
                    } else {
                        event.rejectDrag()
                    }
                }
                ...
                override fun drop(event: DropTargetDropEvent) {
                    event.acceptDrop(dropAction(event.dropAction, event.sourceActions))
    ...
    fun pathsFrom(transferable: Transferable): List<String> {
        ...
        if (out.isEmpty()) {
            wrapperFiles(transferable).forEach { file ->
                out.add(file.absolutePath)
            }
        }
    ...
    private fun wrapperFiles(transferable: Transferable): List<File> {
        return try {
            val flavor = FileCopyPasteUtil.createJvmDataFlavor(TransferableWrapper::class.java)
                ?: return emptyList()
            if (!transferable.isDataFlavorSupported(flavor)) {
                return emptyList()
            }
            val wrapper = transferable.getTransferData(flavor) as? TransferableWrapper
                ?: return emptyList()
            val listed = wrapper.asFileList()
            if (!listed.isNullOrEmpty()) {
                return listed
            }
            wrapper.psiElements?.mapNotNull { element ->
                val virtual = element.containingFile?.virtualFile ?: return@mapNotNull null
                if (virtual.isInLocalFileSystem) File(virtual.path) else null
            } ?: emptyList()
        } catch (_: Throwable) {
            emptyList()
        }
    ...
    private fun dropAction(dropAction: Int, sourceActions: Int): Int {
        if (dropAction and DnDConstants.ACTION_COPY != 0) {
            return DnDConstants.ACTION_COPY
        }
        if (sourceActions and DnDConstants.ACTION_COPY != 0) {
            return DnDConstants.ACTION_COPY
        }
        if (sourceActions and DnDConstants.ACTION_MOVE != 0) {
    ...
    private fun hasFiles(transferable: Transferable): Boolean {
        return try {
            FileCopyPasteUtil.isFileListFlavorAvailable(transferable.transferDataFlavors) ||
                ...
                FileCopyPasteUtil.createJvmDataFlavor(TransferableWrapper::class.java)
                    ?.let { transferable.isDataFlavorSupported(it) } == true
        } catch (_: Throwable) {
            false
        }
    }
